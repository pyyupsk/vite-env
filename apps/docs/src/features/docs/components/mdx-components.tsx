...
export const mdxComponents = {
  ...
  a: ({ href = '', children, ...props }: ComponentPropsWithoutRef<'a'>) => {
    const cls = 'text-accent-text underline underline-offset-2 hover:text-accent-hover transition-colors duration-fast'
    if (href.startsWith('/')) {
  ...
  ul: (props: ComponentPropsWithoutRef<'ul'>) => (
    <ul className="text-[14.5px] text-text-muted leading-[1.7] mb-4 pl-5 list-disc space-y-1" {...props} />
  ),
  ol: (props: ComponentPropsWithoutRef<'ol'>) => (
    <ol className="text-[14.5px] text-text-muted leading-[1.7] mb-4 pl-5 list-decimal space-y-1" {...props} />
  ),

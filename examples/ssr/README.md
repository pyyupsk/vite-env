...
Vite SSR app where the server entry imports `virtual:env/server` (all vars) and the client entry imports `virtual:env/client` (client subset only). The `allowedServerEnvironments: ['ssr']` option gates who may touch the server module — the client environment is always blocked.

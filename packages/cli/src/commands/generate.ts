...
function getClientPrefixLabel(def: EnvDefinition): string {
  if (def.clientPrefix) {
    const prefixes = Array.isArray(def.clientPrefix) ? def.clientPrefix : [def.clientPrefix];
    return `${prefixes.join(" or ")} prefix`;
  }
  return "VITE_ prefix";
}
...
export const generateCommand = defineCommand({
  ...
  async run({ args }) {
    ...
    const def = (await loadDef(configPath)) as EnvDefinition;

    const clientPrefixLabel = getClientPrefixLabel(def);
    ...
    const sections: [string, z.ZodRawShape | undefined][] = [
      [`Client variables (${clientPrefixLabel} — available everywhere)`, def.client],
...
export function buildHint(schema: z.ZodTypeAny): string {
  ...
  if (inner instanceof z.ZodBoolean) return "boolean (true | false | 1 | 0)";
  if (inner instanceof z.ZodEnum) return `enum: ${(inner.options as string[]).join(" | ")}`;
  if (inner instanceof z.ZodPipe) return buildHint(inner.def.out as unknown as z.ZodTypeAny);
  return "string";
}
...
function unwrap(schema: z.ZodTypeAny): z.ZodTypeAny {
  if (schema instanceof z.ZodOptional) return unwrap(schema.unwrap() as unknown as z.ZodTypeAny);
  if (schema instanceof z.ZodDefault)
    return unwrap((schema.def as { innerType: unknown }).innerType as z.ZodTypeAny);
  if (schema instanceof z.ZodPipe) return unwrap(schema.def.out as unknown as z.ZodTypeAny);
  return schema;
}

...
describe("checkServerModuleAccess", () => {
  it("returns allowed when envName is in serverEnvironments", () => {
    expect(checkServerModuleAccess("ssr", ["ssr"], "error", undefined)).toEqual({ allowed: true });
  });

  it("returns GuardFail with correct shape for error mode", () => {
    expect(checkServerModuleAccess("client", ["ssr"], "error", "src/foo.ts")).toEqual({
      allowed: false,
      mode: "error",
      envName: "client",
      importer: "src/foo.ts",
  ...
  it("returns GuardFail with mode warn", () => {
    expect(checkServerModuleAccess("client", ["ssr"], "warn", "src/foo.ts")).toEqual({
      allowed: false,
      mode: "warn",
      envName: "client",
      importer: "src/foo.ts",
  ...
  it("returns GuardFail with mode stub and undefined importer", () => {
    expect(checkServerModuleAccess("client", ["ssr"], "stub", undefined)).toEqual({
      allowed: false,
      mode: "stub",
      envName: "client",
      importer: undefined,
  ...
  it("allows custom serverEnvironments — workerd allowed, client blocked", () => {
    expect(checkServerModuleAccess("workerd", ["workerd"], "error", undefined)).toEqual({
      allowed: true,
    });
    expect(checkServerModuleAccess("client", ["workerd"], "error", undefined)).toMatchObject({
      allowed: false,
    });
  });

  it("default serverEnvironments pattern — ssr allowed, client blocked", () => {
    expect(checkServerModuleAccess("ssr", ["ssr"], "warn", undefined)).toEqual({ allowed: true });
    expect(checkServerModuleAccess("client", ["ssr"], "warn", undefined)).toMatchObject({
      allowed: false,
    });
  });
});

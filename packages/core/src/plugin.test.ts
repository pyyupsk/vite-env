...
vi.mock("./standard", () => ({
  ...
  validateStandardEnv: vi
    .fn()
    .mockResolvedValue({ success: true, data: { VITE_X: "val" }, errors: [] }),
}));
...
describe("viteEnv plugin", () => {
  ...
  it(String.raw`should resolve virtual module ids`, () => {
    const plugin = ViteEnv({ configFile: "env.mjs" }) as Plugin & {
  ...
  it("should throw when config file is missing", async () => {
    ...
    await expect(plugin.configResolved(createMockConfig(tmpDir))).rejects.toThrow(
      "[vite-env] Could not load env definition file",
    );
  });
  ...
  describe("configureServer", () => {
    it("should watch .env files", async () => {
      ...
      expect(mockServer.watcher.add).toHaveBeenCalledWith(path.join(tmpDir, ".env*"));
...
describe("guard integration", () => {
  ...
  beforeEach(async () => {
    ...
    writeEnvFile(tmpDir);
    plugin = ViteEnv({ configFile: "env.mjs", onClientAccessOfServerModule: "warn" });
    mockConfig = createMockConfig(tmpDir);
    await plugin.configResolved(mockConfig);
    await plugin.buildStart();
  });
  ...
  it("resolveId from client environment records GuardFail (warn mode)", () => {
  ...
  it("serverEnvironments emits deprecation warning", async () => {
    const warnSpy = vi.spyOn(console, "warn").mockImplementation(() => {});
    ViteEnv({ configFile: "env.mjs", serverEnvironments: ["ssr"] });
    expect(warnSpy).toHaveBeenCalledWith(
      expect.stringContaining("serverEnvironments is deprecated"),
    );
    warnSpy.mockRestore();
  });

  it("allowedServerEnvironments does not emit deprecation warning", async () => {
    const warnSpy = vi.spyOn(console, "warn").mockImplementation(() => {});
    ViteEnv({ configFile: "env.mjs", allowedServerEnvironments: ["ssr"] });
...
describe("serverRuntime option", () => {
  ...
  async function setupPlugin(
    ...
    fs.writeFileSync(
      filePath,
      "export default { client: { VITE_API_URL: {} }, server: { DATABASE_URL: {}, JWT_SECRET: {} } }\n",
    );
  ...
  it("should emit process.env references (not inlined values) when serverRuntime is process-env", async () => {
    ...
    expect(mod.code).not.toContain("postgresql://test");
    expect(mod.code).not.toContain('"secret"');
    expect(mod.code).not.toContain("https://api.test");
  });

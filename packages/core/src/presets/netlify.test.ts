...
describe("netlify preset", () => {
  ...
  describe("schema validation", () => {
    ...
    it("DEPLOY_URL accepts a full https:// URL", () => {
      expect(
        netlify.server.DEPLOY_URL.safeParse("https://deploy-abc123--mysite.netlify.app").success,
      ).toBe(true);
    });

    it("DEPLOY_URL rejects a bare hostname without scheme", () => {
      expect(netlify.server.DEPLOY_URL.safeParse("deploy-abc123--mysite.netlify.app").success).toBe(
        false,
      );
    });
    ...
    it("INCOMING_HOOK_URL accepts a full https:// URL when present", () => {
      expect(
        netlify.server.INCOMING_HOOK_URL.safeParse("https://api.netlify.com/hooks/abc").success,
      ).toBe(true);
    });
